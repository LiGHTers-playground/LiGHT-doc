...
To setup the credentials, we must access the scratch in `jumphost.rcp.epfl.ch` using ssh. Note that you can also replace the `$GASPAR` by your EPFL mail address. The password is your GASPAR credentials:
```bash
